	...
	public LoginJFrame() {
		...
		jButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Data data=new Data(account.getText(), "", passwd.getText(), Data.LOGIN, null);
				try {
					Socket socket=new Socket("127.0.0.1", 8888);
					//此处切莫画蛇添足放一个ObjectOutputStream,因为send里已经有了，不能同时占用
					Tools.send(data, socket);
					Data data1=Tools.recieve(socket);
					HomeJFrame homeJFrame=null;
					if(data1.getType()==Data.LOGIN) {
						System.out.println(data1.getMessage());
						homeJFrame=new HomeJFrame(socket,data1.getFriends(),account.getText());
